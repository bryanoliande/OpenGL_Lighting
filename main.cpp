...
void display(void)
{
  ...
  // light transformations
    ...
    for (i = 0; i < 3; i++)
    {
        m[i][3] = 0.0;
        m[3][i] = 0.0;
        for (j = 0; j < 3; j++) {m[i][j] = M[i*4+j];}
    }
    m[3][3] = 1.0;
    multVector(light_pos, m, light_position);
    glPopMatrix();
...
int main(int argc, char **argv)
{
  ...
  filename = "/Users/bryanoliande/Desktop/Portfolio/C++_Programming_Examples/Graphics_Assignment_3/bunny.ply";
