...
void PLYObject::draw()
{
    ...
    else
    {
        ...
        Vector3f V, L, H, E, N, tempVector3f, amb, dif, spe, total, vertexColor;
        
        
        glDisable(GL_LIGHTING); //disabling GL_LIGHTING b/c we are implementing our own
        for (int v=0; v < nv; v++)
        {
            ...
            // Instead of this, use the Phong illumination equation to find the colour
            
            
       
            // This block of code was taken from main.cpp.
            // in lecture we were told to multiply V by the modelView matrix.
            // The only way to do that is to store modelView in a Matrix4f variable and then multiply it
            float M[16];
            Matrix4f modelView;
            ...
            multVector(L, modelView, light_pos);
            normalize(L); // make L a unit vector
            
        
            // V = view direction
          // normalize(viewer_pos);
         
            multVector(V, modelView, viewer_pos);
            normalize(V); // make V a unit vector
        
        
          
            
            // H = L + V / magnitude(L + V) ... H is the half vector between L and V
            add(H, L, V);
            
            
         float d = normalize(H); //normalize returns a float, can't store it in H... to be used in fatt distance between the light's position and the vertex
            float k1;
            float k2;
            float k3;
            ...
            float fatt = 1 / (k1 + (k2 * d) + (k3 * (d * d))); //attenuation equation
            
            
            // Only using one light (GL_LIGHT0)
            ...
            Vector4f materialEmission;
            glGetMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, materialEmission);
            add(vertexColor,materialEmission);
            ...
            multVectors(tempVector3f, globalAmbient, ambient);
            
            
            // + fatt * [(light ambient)*(material ambient)] **We are distributing fatt at this point **
            multVectors(amb, lightAmbient, ambient);
            scale(amb, fatt);
            
            add(vertexColor, tempVector3f, amb); //it wasn't working when I tried using add with 2 arguments. need tempVector3f
            
            
            //+ fatt * (max {L · N, 0}) * diffuselight * diffusematerial
            ...
            multVector(N, modelView, normals[v]);
            normalize(N);
            
            
            // L = unit vector towards light source
            // recomputing L after computing N (was told to do so in lecture)
            normalize(H);
            for(int i = 0; i < 3; i++)
            {
                L[i]= H[i];
            ...
            float LdotN= dotProd(L, N);
            Vector4f lightDiffuse;
            glGetLightfv(GL_LIGHT0, GL_DIFFUSE, lightDiffuse);
            
            //if it's 0 nothing is added to the equation
            if (LdotN > 0.0)
            {
                // light diffuse * material diffuse * LdotN *fatt
                multVectors(dif,lightDiffuse,diffuse);
                scale(dif,LdotN);
                scale(dif, fatt); ///test
                add(vertexColor, vertexColor, dif);
            }
            
            //+ (max{H.N,0})^n *(light specular)*(material specular) * fatt
            
            
            float HdotN= dotProd(H, N);
            Vector4f lightSpecular;
            ...
            if(HdotN > 0)
            {
                max = pow(HdotN, shininess[0]); //{max{H.N,0}^n  shininess[0] is the specular exponent
            ...
            //Performing the lighting calculation for each vertex
            
            //RGB values represented in 24 bits per pixel (bpp), also called Truecolor, are normally specified with three integers
            //between the values 0 and 255. Each of these three integers represents the intensities of red, green, and blue
            //respectively.
            
            //Multiply each color value by 255 (It's intensity * 255) = how much RGB intensity it has
            
            colors[v][0] = vertexColor[0] * 255; //red
            colors[v][1] = vertexColor[1] * 255; //green
            colors[v][2] = vertexColor[2] * 255; //blue
